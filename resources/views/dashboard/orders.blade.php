        ...
        <table>
            ...
            <tbody id="orders-table-body">
                @foreach ($orders as $o)
                    <tr class="{{ $loop->even ? 'bg_gray_200' : 'bg-white' }}">
                        <td><div class="text-center iteration-number">{{ $startNumber + $loop->iteration }}</div></td>
                        @php
                                ...
                                <div class="btn_orders_action">
                                    ...
                                    @if($pelanggan)
                                        <a href="{{ route('pelanggan.nota', $pelanggan->id) }}">
                                            <button class="btn_nota bg-yellow-500">Lihat Nota</button>
                                        </a>
                                    @endif
            ...
            <tfoot>
                <tr class="bg-gray-400 font-bold">
                    ...
                    <td id="total_hari">
                        <div class="text-center">
                        <b>{{ (float) $totals?->total_hari }}</b></div>
                    </td>
                    <td id="total_deadline">
                        <div class="text-center">
                        <b>{{ (float) $totals?->total_deadline }}</b></div>
                    </td>
                    <td id="total_setting">
                        <div class="text-center">
                        <b>{{ $totals?->total_setting }}</b></div>
                    </td>
                    <td id="total_print">
                        <div class="text-center">
                        <b>{{ $totals?->total_print }}</b></div>
                    </td>
                    <td id="total_sisa_print">
                        <div class="text-center">
                        <b>{{ $totals?->total_sisa_print }}</b></div>
                    </td>
                    <td id="total_press">
                        <div class="text-center">
                        <b>{{ $totals?->total_press }}</b></div>
                    </td>
                    <td id="total_sisa_press">
                        <div class="text-center">
                        <b>{{ $totals?->total_sisa_press }}</b></div>
                    </td>
                    <td id="total_cutting">
                        <div class="text-center">
                        <b>{{ $totals?->total_cutting }}</b></div>
                    </td>
                    <td id="total_sisa_cutting">
                        <div class="text-center">
                        <b>{{ $totals?->total_sisa_cutting }}</b></div>
                    </td>
                    <td id="total_jahit">
                        <div class="text-center">
                        <b>{{ $totals?->total_jahit }}</b></div>
                    </td>
                    <td id="total_sisa_jahit">
                        <div class="text-center">
                        <b>{{ $totals?->total_sisa_jahit }}</b></div>
                    </td>
                    <td id="total_finishing">
                        <div class="text-center">
                        <b>{{ $totals?->total_finishing }}</b></div>
                    </td>
                    <td id="total_sisa_finishing">
                        <div class="text-center">
                        <b>{{ $totals?->total_sisa_finishing }}</b></div>
                    </td>
                    <td id="total_packing">
                        <div class="text-center">
                        <b>{{ $totals?->total_packing }}</b></div>
                    </td>
                    <td id="total_sisa_packing">
                        <div class="text-center">
                        <b>{{ $totals?->total_sisa_packing }}</b></div>
                    </td>
                    ...
                    @if(Auth::user()->role === 'admin')
                    <td class="border border-gray-300 px-4 py-2 text-center" colspan="2"></td>
                    @endif
                </tr>
            </tfoot>
        </table>
        @if($orders->hasPages())
            <div class="pagination-wrapper">
                ...
                <div class="pagination-info">
                    ...
                    <strong>{{ $orders->lastItem() ?? 0 }}</strong> dari 
                    <strong>{{ $orders->total() }}</strong> data
                </div>
                ...
                <div class="pagination-nav">
                    ...
                    @else
                        <a href="{{ $orders->previousPageUrl() }}" class="pagination-btn pagination-btn-active">
                            &laquo; Sebelumnya
                        </a>
                    @endif
                    ...
                    <div class="pagination-numbers">
                        ...
                        @if($start > 1)
                            <a href="{{ $orders->url(1) }}" class="pagination-number">1</a>
                            @if($start > 2)
                        ...
                        @for ($page = $start; $page <= $end; $page++)
                            ...
                            @else
                                <a href="{{ $orders->url($page) }}" class="pagination-number">{{ $page }}</a>
                            @endif
                        @endfor
                        ...
                        @if($end < $orders->lastPage())
                            @if($end < $orders->lastPage() - 1)
                                <span class="pagination-dots">...</span>
                            @endif
                            <a href="{{ $orders->url($orders->lastPage()) }}" class="pagination-number">{{ $orders->lastPage() }}</a>
                        @endif
                    </div>
                    
                    <!-- Tombol Next -->
                    @if($orders->hasMorePages())
                        <a href="{{ $orders->nextPageUrl() }}" class="pagination-btn pagination-btn-active">
                            Selanjutnya &raquo;
                        </a>
                    @else
                        <span class="pagination-btn pagination-btn-disabled">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Preserve semua parameter filter saat klik pagination
        const paginationLinks = document.querySelectorAll('.pagination-number[href], .pagination-btn-active[href]');
        ...
        paginationLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                // Salin semua parameter kecuali 'page'
...
<script>
    ...
    function resetOrderModal() {
        ...
        // 2. Reset hidden pelanggan ID
        document.getElementById('savedPelangganId').value = '';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const closeBtn = document.querySelector('#popupModal .dashboard_popup_order_heading button[onclick*="close"]');
        if (closeBtn) {
            closeBtn.setAttribute('onclick', 'closeOrderModal()');
        }
        ...
        if (confirmDeleteOrderBtn) {
            confirmDeleteOrderBtn.onclick = function(e) {
                ...
                fetch(`/dashboard/orders/${currentOrderId}`, {
                    method: "POST", // Gunakan DELETE langsung
                    headers: {
